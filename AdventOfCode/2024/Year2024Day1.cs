...
public class Year2024Day1 : IDay
{
	public Task<string> RunSolution1Async(IList<string> input)
	{
		...
		double answer = 0;
		for (var i = 0; i < listA.Count; i++) answer += Math.Abs(listA[i] - listB[i]);
	...
	public Task<string> RunSolution2Async(IList<string> input)
	{
		...
		foreach (var s in input)
		{
			...
			if (!dict1.ContainsKey(a)) dict1.Add(a, 0);
			if (!dict2.ContainsKey(b)) dict2.Add(b, 0);
		...
		double answer = 0;
		foreach (var kvp in dict1) answer += kvp.Key * kvp.Value * (dict2.TryGetValue(kvp.Key, out var value) ? value : 0);
